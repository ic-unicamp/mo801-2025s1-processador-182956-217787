...
REG_TESTS_SRC_DIR = reg_tests

# Register Dump Tests (Using tb_regdump.v)
REG_DUMP_MEMS = $(wildcard $(REG_TESTS_SRC_DIR)/reg_teste*.mem)

# Derive test names, e.g., from reg_tests/reg_teste10.mem get reg_teste10
REG_DUMP_TEST_NAMES = $(patsubst $(REG_TESTS_SRC_DIR)/%.mem,%,$(REG_DUMP_MEMS))

# Define expected output files (.regok) located in the new source directory
REG_DUMP_TEST_OKS = $(patsubst %.mem,%.regok,$(REG_DUMP_MEMS))

# Define actual output files generated by simulation (these will go into logs/)
REG_DUMP_TEST_ACTUALS = $(patsubst %,$(LOG_DIR)/%.regactual,$(REG_DUMP_TEST_NAMES))
...
run_reg_dump: compile_reg_dump | $(LOG_DIR) $(WAVE_DIR)
	...
	@for test_name in $(REG_DUMP_TEST_NAMES); do \
		...
		if [ ! -f $(REG_TESTS_SRC_DIR)/$${test_name}.mem ]; then \
			echo "Error: Input file $(REG_TESTS_SRC_DIR)/$${test_name}.mem not found!"; \
			exit 1; \
		fi; \
		cp $(REG_TESTS_SRC_DIR)/$${test_name}.mem memory.mem; \
...
test_reg_dump: run_reg_dump
	...
	for test_name in $(REG_DUMP_TEST_NAMES); do \
		...
		EXPECTED_FILE=$(REG_TESTS_SRC_DIR)/$${test_name}.regok; \
